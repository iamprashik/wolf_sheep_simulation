...
Open the address printed in the terminal, normally [http://localhost:8765](http://localhost:8765).
...
Run each comparison with the same seed so differences are caused by the selected parameter rather than a different random sequence.
...
Built by **Prashik Koirala** using the [Mesa agent-based modelling framework](https://mesa.readthedocs.io/) and its Wolf–Sheep example as the foundation.
